(() => {
  ...
  const EN = {
    ...
    model3Title: "Import models or connect an API",
    model3Body: "Your own models need separate validation. External APIs use the provider's privacy and billing terms.",
  ...
  const JA = {
    ...
    model3Title: "モデルの取り込みと外部API",
    model3Body: "持ち込みモデルは別途検証が必要です。外部APIには提供者のプライバシー・料金条件が適用されます。",
    installEyebrow: "START SAFELY",
  ...
  const ZH = {
    ...
    model3Title: "导入模型或连接外部 API",
    model3Body: "自行导入的模型需要另外验证。外部 API 适用提供商的隐私与计费条款。",
    installEyebrow: "START SAFELY",
    installTitle: "三步安全开始",
