...
**v1.3.8 업데이트 안정화:** 전송 중단 시 받은 데이터를 보존하고 검증된 HTTP Range로 이어받습니다. 진행 알림과 저장공간 안내도 개선했습니다. Runtime 2.0의 CPU 튜닝·지원 Adreno GPU 가속·오프라인 의미 기억을 포함하며 APK는 약 **148 MB**입니다. 대화용 GGUF 모델은 별도로 선택합니다. [릴리즈 노트](releases/v1.3.8.md).
